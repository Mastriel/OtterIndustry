import type {Fetchable, ReactiveTrigger, SubscribeFunction} from "../../utils/writableUtils";
import {fetchable, reactiveTrigger} from "../../utils/writableUtils";
import type {Modifier, ModifierMap, ReactiveModifierMap} from "../utils/modifiers";
...
export class Resource implements Readable<Resource>, Serializable<DataResource>, Ticking {
    ...
    constructor({name, nameLong, id, max, description, color, visible = alwaysVisible}: ResourceOptions) {
        this.name = name
    ...
    public readonly restoreFrom = (data: DataResource) => {
        ...
        this.maxModifiers = reactiveModifierMap(new Map([...Object.entries(data.maxModifiers), ...this.maxModifiers.entries()]))
        this.generationModifiers = reactiveModifierMap(new Map([...Object.entries(data.generationModifiers), ...this.generationModifiers.entries()]))
    ...
    public readonly serialize = () : DataResource => {
        return {
            ...
            maxModifiers: Object.fromEntries(this.maxModifiers.getSerializedValues()),
            generationModifiers: Object.fromEntries(this.generationModifiers.getSerializedValues())
        }
    }
