...
export class ResourceRegistry implements IResourceRegistry {
    ...
    public readonly getResource = (type: new () => Resource): Result<Resource> => {
        let resource = this._registeredResources.find(it => it instanceof type)
        return resultAuto(resource, `invalid resource type: ${type.name}`)
    }

    public readonly getResourceById = (id: string): Result<Resource> => {
        let resource = this._registeredResources.find(it => it.id == id)
        return resultAuto(resource, `invalid resource id: ${id}`)
    }

    public readonly loadAll = () => {
        let structure = <ResourceStructure>JSON.parse(localStorage.getItem("resources"))
        if (structure == undefined) {
            return
        }
        for (let resource of this._registeredResources) {
            if (structure[resource.id]) resource.restoreFrom(structure[resource.id])
        }
    }
