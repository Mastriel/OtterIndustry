...
export function ticking(run: (delta: number) => void, idealInterval: number) : Ticking {
    return {
        tick: run,
        idealInterval
    }
...
export class IntervalTicker implements Ticker {
    ...
    public readonly track = (ticking: Ticking) => {
        ...
        let intervalId = window.setInterval(() => ticking.tick(interval), interval*1000)
    ...
    public get timePlayed() { return this._timePlayed }
}