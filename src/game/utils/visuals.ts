...
export const enableTick = (game: Game, parent: { isVisible: boolean }, requirement: VisibilityRequirement) => {
    game.Ticker.track(ticking(() => {
...
export const requiresDiscovery = (resourceId: string) => {
    return (game: Game) => game.Resources.getResourceById(resourceId).get().amountEverObtained > 0
}
