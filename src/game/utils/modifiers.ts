...
export const modifier = (name: string, amount: number, operation: ModifierOperation, quantity: number = 1, priority: number = 1, serialize: boolean = false, duration?: number) : Modifier => {
...
export const displayOf = (modifier: Modifier) => {
    switch (modifier.operation) {
        ...
        case "-":
            return "-" + display(modifier.amount * modifier.quantity)
        case "*":
            return "+" + display(((modifier.amount * modifier.quantity) - 1) * 100) + "%"
        case "/":
            return "-" + display(1 / (modifier.amount * modifier.quantity)) + "%"
        case "^":
            return "^" + display(modifier.amount * modifier.quantity)
...
export const modifierMap = (from?: Map<string, Modifier>) : ModifierMap => {
    ...
    let modifierMap : ModifierMap = Object.assign(map, {
        apply: (n: number) : number => {
            let array = Array.from(map.values()).sort((a, b) => b.priority - a.priority)
            for (let modifier of array) {
                switch (modifier.operation) {
                    ...
                    case "^":
                        ...
                        break;
                    default:
                        console.warn(`Invalid Modifier Operation: ${modifier.operation}`)
                        break;
                }
            }
            return n
        ...
        getSerializedValues(): [string, Modifier][] {
            return Array.from(map.entries()).filter(it => it && it[1]?.serialize == true)
        },
        getSortedArray(): [string, Modifier][] {
            return Array.from(map.entries()).sort((a, b) => b[1].priority - a[1].priority)
        },
    })
...
export const reactiveModifierMap = (from?: Map<string, Modifier>) : ReactiveModifierMap => {
