    ...
    <p>
        <Tooltip title={$resource.nameLong} emoji="resource">
            <svelte:fragment slot="element">
                ...
                {#if isFull}
                    <span class="text-yellow-200">{display($resource.amount, 2)}{maxDisplay}
                        <Emoji id="warning" color="#fef08a" /></span>
                {:else}
