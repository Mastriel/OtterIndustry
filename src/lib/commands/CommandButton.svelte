...
{#if $command.isVisible}
    <Tooltip title={$command.name} emoji="command">
        <svelte:fragment slot="element">
            <button on:click={onClick} class="text-center h-20 bg-zinc-900 rounded-md border-gray-400 border-2 w-full pt-2 pb-2" class:cant-run={!$command.canRun}>
                    ...
                    <span class="block w-full mr-6 ml-6 h-1 bg-gray-600">
                        {#key animationState}
                            ...
                            {:else if animationState === "cooldown"}
                                <span in:drainProgressBar class="block bg-yellow-400 h-full" class:bg-green-400={$command.cooldownIsBuff} style="width: 0"></span>
                            {:else}
                                <span in:fadeOutBar class="block bg-green-400 h-full" style="width: 100%; opacity: 0"></span>
                            {/if}
                        {/key}
                    </span>
                </span>
